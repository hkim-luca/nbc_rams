import {
  Viewer, Entity, Color, Cartesian3, NearFarScalar,
} from 'cesium';
import type { PuffData } from '../store';

const puffEntities: Map<number, Entity> = new Map();
const REF_LAT = 37.5;
const REF_LON = 127.0;
...
export function updatePuffs(viewer: Viewer, puffs: PuffData[]): void {
  ...
  // Update or create entities for current puffs
  for (const puff of puffs) {
    const [lat, lon] = metersToDegrees(puff.x, puff.y);
    // Normalize concentration-based color
    ...
    if (!entity) {
      ...
      (entity.point!.pixelSize as any) = Math.max(2, puff.sig_y * 0.05);
